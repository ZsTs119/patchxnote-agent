...
assert.strictEqual(resolveRedirectURL("https://github.com/ZsTs119/patchnote-agent/releases/download/v0.1.1/checksums.txt", "/download"), "https://github.com/download");
...
assert.match(pathHint("C:\\PatchNote", "windows"), /SetEnvironmentVariable/);
...
assert.strictEqual(isInstallDirOnPath("C:\\PatchNote", "C:\\Windows;C:\\PatchNote", "windows"), true);

const binary = Buffer.from("patchnote-binary-fixture");
const checksum = crypto.createHash("sha256").update(binary).digest("hex");
assert.doesNotThrow(() => verifyChecksum(binary, "patchnote_0.1.1_linux_amd64", `${checksum}  patchnote_0.1.1_linux_amd64\n`));
assert.throws(() => verifyChecksum(binary, "patchnote_0.1.1_linux_amd64", `bad  patchnote_0.1.1_linux_amd64\n`), /checksum mismatch/);
...
assert.match(dryRun.stdout, /patchnote_0.1.1_linux_amd64/);
...
assert.match(dryRun.stdout, /PATCHNOTE_AUTH_INSECURE_FILE_KEYCHAIN/);
assert.doesNotMatch(dryRun.stdout, /access_token|refresh_token|otp|sk_|protocol_mac/i);

for (const [platform, arch, asset] of [
  ["linux", "x64", "patchnote_0.1.1_linux_amd64"],
  ["darwin", "arm64", "patchnote_0.1.1_darwin_arm64"],
  ["win32", "x64", "patchnote_0.1.1_windows_amd64.exe"]
]) {
